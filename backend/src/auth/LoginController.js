    ...
    if (Number(user.tipo_usuario.id_tipo_usuario) == 1) {
      ...
      const inscribe = await prisma.inscribe.findFirst({
        where: {
          ...
          id_estado_practica: {
            not: 3, // Excluye el estado de práctica "finalizado"
          },
        },
        include: {
          asignatura: true,
      ...
      if (!inscribe) {
        return res.status(401).json({
          mensaje: "No tienes inscrita la práctica en intranet o ya está finalizada",
        });
      }

      const asignatura = await prisma.asignatura.findFirst({
        where: {
          id_asignatura: Number(inscribe.id_asignatura),
        },
      });
      if (!asignatura) {
        return res.status(401).json({
