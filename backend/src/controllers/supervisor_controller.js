...
const crear_supervisor = async (req, res)=>{
    ...
    try {
        ...
        if(!supervisor){
            return res.status(400).json({message:'No se ha podido crear el supervisor'})
        }
            return res.status(200).json({message:'El supervisor se ha añadido correctamente',supervisor:supervisor})
        } catch (error) {
           
            return res.status(400).json({message:'No se ha podido añadir el supervisor'})
        }
    }
    ...
    const obtener_supervisores = async(req, res)=>{
        try {
            ...
            if(supervisor.length == 0){
                return res.status(200).json({message:'Error, no existe lista de supervisores'})
            }
    ...
    const obtener_supervisor = async(req, res)=>{
        ...
        try {
            ...
            if(!supervisor){
                return res.status(200).json({message:'Error, no existe este supervisor'})
            }
            return res.status(200).json({
    ...
    const eliminar_supervisor = async(req, res)=>{
        try {
            ...
            if(!supervisor){
                return res.status(200).json({message: 'No se ha encontrado a ningun supervisor con este ID'})
            }
            ...
            return res.status(400).json({message:'Error, no se ha podido eliminar el supervisor',
            error:error.stack
        })
        }
    }
    ...
    const actualizar_supervisor = async(req, res)=>{
        try {
            ...
            if(!supervisorActualizado){
                return res.status(200).json({message:'Error al actualizar al supervisor'})
            }
            return res.status(200).json({
    ...
    module.exports = {obtener_supervisor, obtener_supervisores, crear_supervisor, eliminar_supervisor, actualizar_supervisor}
